 ...
 */
export type ChatProvider = 'copilot' | 'openai';
...
const envSchema = z
  .object({
     ...
     */
    CHAT_PROVIDER: z.enum(['openai', 'copilot']).optional().default('openai'),
...
export function validateEnv(): EnvConfig {
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      console.error(
        '\n💡 Set CHAT_PROVIDER (openai|copilot) plus the matching auth env var (OPENAI_API_KEY for openai, optional COPILOT_GITHUB_TOKEN for copilot).',
      );
      process.exit(1);
    }
    throw error;
