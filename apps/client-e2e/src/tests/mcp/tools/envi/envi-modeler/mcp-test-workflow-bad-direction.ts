...
/**
 * Test workflow validation with bad edge direction (connecting wrong parameters)
 */
export const RunMCPTestWorkflowBadDirection: RunnerFunction = async (_init) => {
  ...
  // Invalid edges: connecting input parameter to output parameter (wrong direction)
  const edges = [
    {
      from: 'input_params',
    ...
    },
    {
      ...
      from_parameters: ['input_raster1'], // Invalid: trying to connect input instead of output
      to: 'layerstack_task',
      to_parameters: ['output_raster'], // Invalid: connecting to output instead of input
    },
    {
  ...
  const outputPath = join(tmpdir(), `test_bad_direction_${Date.now()}.model`);
