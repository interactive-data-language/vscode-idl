...
export const OpenIDLNotebookExample: RunnerFunction = async (init) => {
  // make a new notebook
  await vscode.commands.executeCommand(IDL_COMMANDS.NOTEBOOKS.OPEN_WHAT_IS_IDL);
  ...
  expect(CleanPath(editor?.notebook?.uri.fsPath) || '').toEqual(
    join('EXAMPLE_NOTEBOOKS', '01-IDL for beginners', '02-IDL Basics', 'hello-world-idl.idlnb');
  );
};
