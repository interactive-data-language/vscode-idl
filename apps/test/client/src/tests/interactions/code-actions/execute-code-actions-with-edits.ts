...
export const ExecuteCodeActionsWithEditForPROFile: RunnerFunction = async (
  ...
  const doc = await OpenFileInVSCode(
    GetExtensionPath(
      'apps/test/idl/client-e2e/problems/code_actions_fix_before.pro',
    ),
  );
  ...
  expect(doc.getText()).toEqual(
    readFileSync(
      GetExtensionPath(
        'apps/test/idl/client-e2e/problems/code_actions_fix_after.pro',
      ),
      'utf-8',
    ),
  );
...
export const ExecuteCodeActionsWithEditForNotebook: RunnerFunction = async (
  ...
  const nb = await OpenNotebookInVSCode(
    GetExtensionPath(
      'apps/test/idl/client-e2e/problems/code-actions-fix.idlnb',
    ),
  );
  ...
  expect(SplitCode(doc.getText())).toEqual(
    SplitCode(
      readFileSync(
        GetExtensionPath(
          'apps/test/idl/client-e2e/problems/code_actions_fix_cell_after.pro',
        ),
        'utf-8',
      ),
    ),
