...
export const IDLDisableAllFromComments: RunnerFunction = async (init) => {
  const doc = await OpenFileInVSCode(
    GetExtensionPath(
      'apps/test/idl/client-e2e/problems/idl_disable_all_before.pro',
    ),
  );

  // short pause to make sure we open and parse
  await Sleep(TEST_CLIENT_CONFIG.DELAYS.SETTINGS);
  ...
  await ReplaceDocumentContent(
    ...
    readFileSync(
      GetExtensionPath(
        'apps/test/idl/client-e2e/problems/idl_disable_all_after.pro',
      ),
      'utf-8',
    ),
  );
...
export const IDLDisableLinesFromComments: RunnerFunction = async (init) => {
  const doc = await OpenFileInVSCode(
    GetExtensionPath(
      'apps/test/idl/client-e2e/problems/idl_disable_lines_before.pro',
    ),
  );
  ...
  await ReplaceDocumentContent(
    ...
    readFileSync(
      GetExtensionPath(
        'apps/test/idl/client-e2e/problems/idl_disable_lines_after.pro',
      ),
      'utf-8',
    ),
  );
