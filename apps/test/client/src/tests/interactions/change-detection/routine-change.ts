...
export const RoutineChange: RunnerFunction = async (init) => {
  const doc1 = await OpenFileInVSCode(
    GetExtensionPath('apps/test/idl/client-e2e/change-detection/file1.pro'),
  );

  const doc2 = await OpenFileInVSCode(
    GetExtensionPath('apps/test/idl/client-e2e/change-detection/file2.pro'),
    true,
    true,
  );
  ...
  await ReplaceDocumentContent(
    ...
    readFileSync(
      GetExtensionPath(
        'apps/test/idl/client-e2e/change-detection/file2-after.pro',
      ),
      'utf-8',
    ),
  );
