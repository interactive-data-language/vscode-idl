...
export const AddDocs: RunnerFunction = async (init) => {
  const doc = await OpenFileInVSCode(
    GetExtensionPath('apps/test/idl/client-e2e/docs/docs_test.pro'),
  );
  ...
  expect(doc.getText()).toEqual(
    readFileSync(
      GetExtensionPath('apps/test/idl/client-e2e/docs/docs_test_out.pro'),
      'utf-8',
    ),
  );
  ...
  await ReplaceDocumentContent(
    doc,
    readFileSync(
      GetExtensionPath('apps/test/idl/client-e2e/docs/docs_test_edit.pro'),
      'utf-8',
    ),
  );
  ...
  expect(doc.getText()).toEqual(
    readFileSync(
      GetExtensionPath('apps/test/idl/client-e2e/docs/docs_test_edit_out.pro'),
      'utf-8',
    ),
  );
};
