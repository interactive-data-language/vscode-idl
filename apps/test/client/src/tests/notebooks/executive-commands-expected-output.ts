...
export const ExecutiveCommandsExpectedOutput: RunnerFunction = async (init) => {
  const nbUri = GetExtensionPath(
    'apps/test/idl/client-e2e/notebooks/executive-commands/quiet-works-right.idlnb',
  );

  const expectedUri = GetExtensionPath(
    'apps/test/idl/client-e2e/notebooks/executive-commands/quiet-works-right-output.idlnb',
  );
  ...
  await Sleep(TEST_CLIENT_CONFIG.DELAYS.NOTEBOOK_PARSE);
