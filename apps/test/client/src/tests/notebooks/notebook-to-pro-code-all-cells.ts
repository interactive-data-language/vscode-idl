 ...
 */
export const NotebookToProCodeAllCells: RunnerFunction = async (init) => {
  const nbUri = GetExtensionPath(
    'apps/test/idl/client-e2e/notebooks/notebook-to-pro-code/test-notebook.idlnb',
  );

  const expectedUri = GetExtensionPath(
    'apps/test/idl/client-e2e/notebooks/notebook-to-pro-code/test_notebook_all_cells.pro',
  );
