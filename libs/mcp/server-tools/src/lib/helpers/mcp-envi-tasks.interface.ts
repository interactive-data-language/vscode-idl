...
3 ${MCP_TOOL_LOOKUP.ENVI_RUN_TASK} - Finally, use this tool to run a capability, providing the correct parameters as JSON (matching the schema you retrieved).

💡 RULES:
- Always call ${MCP_TOOL_LOOKUP.ENVI_LIST_TASKS} first when you need to check what capabilities are available.
- Always call ${MCP_TOOL_LOOKUP.ENVI_GET_TASK_PARAMETERS} before trying to run a capability, so you know what inputs are required.
