...
import {
  ...
  STRUCTURE_TOKENS,
  TERNARY_TOKENS,
} from './def-groups-full.interface';
import { SUB_DEFS_FULL } from './sub-defs-full.interface';
...
describe('Validates all tokens are NOT global', () => {
  ...
  it('validates all sub defs', () => {
    const groups = Object.keys(SUB_DEFS_FULL);

    // process each group
    for (let i = 0; i < groups.length; i++) {
      const group = SUB_DEFS_FULL[groups[i]];
