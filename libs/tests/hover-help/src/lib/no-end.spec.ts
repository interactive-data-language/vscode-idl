...
describe(`[auto generated] Correctly does not provide hover help`, () => {
  it(`[auto generated] for end tokens but does for the beginning`, async () => {
    ...
    const expectedFound_0: string[] = [
      '#### [plot](https://www.nv5geospatialsoftware.com/docs/PLOT.html)',
      '',
      '```idl',
      'result = plot( [ x ], [ y ], [ format ], [ equation ], [ axis_style = String ], [ /buffer ], [ /current ], [ /device ], [ dimensions = any ], [ layout = any ], [ location = any ], [ margin = any ], [ no_toolbar = any ], [ nodata = String ], [ /overplot ], [ widgets = any ], [ _extra = any ], [ /test ])',
      '```',
      '',
